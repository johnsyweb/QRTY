...
let decodedValues = [];
const imageUtils = window.QRTY_IMAGE_UTILS;
if (!imageUtils) {
    throw new Error("Image processing utilities are not initialised.");
}
const { get2dContext, getScaledCanvas, addQuietZone, normaliseImageData, computeOtsuThreshold, binariseImageData, thickenLinearFeatures, getBarcodeImagePayload, } = imageUtils;
...
async function copyToClipboard(value, button) {
    if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        showError(`Clipboard access is unavailable. Please copy this value manually:\n${value}`);
    }
}
...
function scanQRCodesFromCanvas(canvas) {
    ...
    for (;;) {
        ...
        const minX = Math.max(Math.floor(Math.min(...corners.map((point) => point.x)) - margin), 0);
        const maxX = Math.min(Math.ceil(Math.max(...corners.map((point) => point.x)) + margin), width - 1);
        const minY = Math.max(Math.floor(Math.min(...corners.map((point) => point.y)) - margin), 0);
        const maxY = Math.min(Math.ceil(Math.max(...corners.map((point) => point.y)) + margin), height - 1);
...
function scanBarcodesFromCanvas(canvas) {
    ...
    try {
        ...
        const decodeWithBinarizer = (BinarizerClass) => {
            ...
            const binaryBitmap = new ZXingLib.BinaryBitmap(new BinarizerClass(luminanceSource));
        ...
        if (results.length === 0 &&
            ...
            try {
                const pureBinaryBitmap = new ZXingLib.BinaryBitmap(new ZXingLib.GlobalHistogramBinarizer(createLuminanceSource()));
        ...
        if (results.length === 0 &&
            ...
            const rowStep = Math.max(1, Math.floor(processedHeight / (maxSamples + 1)));
            for (let offset = rowStep; offset < processedHeight && rowsToSample.length < maxSamples; offset += rowStep) {
...
function scanCodesFromCanvas(canvas) {
    return mergeUniqueValues(scanQRCodesFromCanvas(canvas), scanBarcodesFromCanvas(canvas));
}
    ...
    reader.onload = (e) => {
        ...
        img.onload = () => {
            ...
            if (values.length > 0) {
                ...
                showError("No QR codes or barcodes found in the image. Please try another image.");
                fileInput.value = "";
            }
        };
        img.onerror = () => {
