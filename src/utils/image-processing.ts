...
function normaliseImageData(imageData: ImageData): void {
...
function binariseImageData(imageData: ImageData): void {
  if (!imageData || !imageData.data) {
    return;
  }
...
function thickenLinearFeatures(imageData: ImageData): void {
  if (!imageData || !imageData.data) {
    return;
  }
...
function getBarcodeImagePayload(
  canvas: HTMLCanvasElement
): { data: Uint8ClampedArray; width: number; height: number } | null {
  if (!canvas) {
    return null;
...
const utils: ImageProcessingUtils = {
  get2dContext,
  getScaledCanvas,
  addQuietZone,
  normaliseImageData,
